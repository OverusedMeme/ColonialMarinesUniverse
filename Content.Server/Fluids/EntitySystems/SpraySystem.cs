...
public sealed partial class SpraySystem : SharedSpraySystem
{
    ...
    private void OnAfterInteract(Entity<SprayComponent> entity, ref AfterInteractEvent args)
    {
        ...
        Spray(entity, clickPos, args.User, args.Target == args.User);
    }
    ...
    /// <summary>
    /// Compatibility entry point for RMC sprays that need their vapor to collide with the user who fired them.
    /// </summary>
    public void Spray(Entity<SprayComponent> entity, EntityUid user, MapCoordinates mapcoord, bool hitUser = false)
    {
        Spray(entity, mapcoord, user, hitUser);
    }

    public override void Spray(Entity<SprayComponent> entity, MapCoordinates mapcoord, EntityUid? user = null)
    {
        Spray(entity, mapcoord, user, false);
    }

    private void Spray(Entity<SprayComponent> entity, MapCoordinates mapcoord, EntityUid? user, bool hitUser)
    {
