...
public sealed partial class SolutionTransferSystem : EntitySystem
{
    ...
    private void OnAfterInteract(Entity<SolutionTransferComponent> ent, ref AfterInteractEvent args)
    {
                ...
                out _))
        {
            ...
            if (transferTime > TimeSpan.Zero)
            {
                ...
                var doAfterArgs = new DoAfterArgs(EntityManager, args.User, transferTime, new SolutionDrainTransferDoAfterEvent(transferAmount), ent.Owner, target: target, used: ent.Owner)
                {
                    BreakOnDamage = true,
                    BreakOnMove = true,
                    NeedHand = true,
                ...
                out var solution))
        {
            ...
            if (transferTime > TimeSpan.Zero)
            {
                ...
                var doAfterArgs = new DoAfterArgs(EntityManager, args.User, transferTime, new SolutionRefillTransferDoAfterEvent(transferAmount), ent.Owner, target: target, used: ent.Owner)
                {
                    BreakOnDamage = true,
                    BreakOnMove = true,
                    NeedHand = true,
        ...
        [NotNullWhen(true)] out Entity<SolutionComponent>? refillable)
    {
        ...
        return ent.Comp1.CanSend && TryGetTransferrableSolutions(ent.Owner, target, out drainable, out refillable, out _);
    }
    ...
    /// <summary>
    /// Transfer from a solution to another using a reusable transfer-data value.
    /// </summary>
    /// <returns>The actual amount transferred.</returns>
