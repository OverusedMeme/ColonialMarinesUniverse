...
public sealed partial class SharedKitchenSpikeSystem : EntitySystem
{
    ...
    private void OnDragDrop(Entity<KitchenSpikeComponent> ent, ref DragDropTargetEvent args)
    {
        ...
        var attempt = new KitchenSpikeHookAttemptEvent(args.User, args.Dragged);
    ...
    private void OnSpikeHookDoAfter(Entity<KitchenSpikeComponent> ent, ref SpikeHookDoAfterEvent args)
    {
        ...
        var attempt = new KitchenSpikeHookAttemptEvent(args.User, args.Target.Value);
        RaiseLocalEvent(ent.Owner, ref attempt);
        if (attempt.Cancelled)
        {
            args.Handled = true;
...
/// <summary>
/// Raised before an entity starts or completes being hooked onto a kitchen spike.
/// </summary>
[ByRefEvent]
public sealed class KitchenSpikeHookAttemptEvent : CancellableEntityEventArgs
{
    public readonly EntityUid User;
    public readonly EntityUid Victim;

    public KitchenSpikeHookAttemptEvent(EntityUid user, EntityUid victim)
    {
        User = user;
        Victim = victim;
    }
