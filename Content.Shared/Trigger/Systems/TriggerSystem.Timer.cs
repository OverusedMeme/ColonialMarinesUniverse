...
{
    /// <summary>
    /// Compatibility helper for callers that configure and start a timer dynamically.
    /// </summary>
        ...
        SoundSpecifier? beepSound)
    {
        if (delay <= 0)
        {
            ...
            Trigger(uid, user, DefaultTriggerKey);
            return;
        }
        ...
        timer.Delay = TimeSpan.FromSeconds(delay);
        ...
        timer.KeyOut = DefaultTriggerKey;
        Dirty(uid, timer);

        ActivateTimerTrigger((uid, timer), user);
    }
