...
public sealed partial class ToolRefinableSystem : EntitySystem
{
    ...
    [Dependency] private EntityQuery<UtensilComponent> _utensilQuery;
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ToolRefinableComponent, AttemptToolRefineEvent>(OnAttemptToolRefine);
        SubscribeLocalEvent<ButcherableComponent, AttemptToolRefineEvent>(OnAttemptButcherRefine);
    }

    #region Subscriptions

    private void OnAttemptToolRefine(Entity<ToolRefinableComponent> ent, ref AttemptToolRefineEvent args)
    {
        var required = ent.Comp.RequiredUtensil;
        if (required == UtensilType.None)
            return;

        if (_utensilQuery.TryComp(args.Using, out var utensil) &&
            (utensil.Types & required) == required)
        {
            return;
        }

        args.IsCancelled = true;
        args.BlockCause = Loc.GetString("refined-slice-verb-message-tool", ("target", ent.Owner));
    }
    ...
    private void OnAttemptButcherRefine(Entity<ButcherableComponent> ent, ref AttemptToolRefineEvent args)
    {
        ...
        // The legacy sharp workflow discarded a completed attempt if the target entered a container.
        if (args.IsCompletion && _container.IsEntityInContainer(ent))
            args.IsCancelled = true;
    }
    ...
    private void OnDoAfter(Entity<ToolRefinableComponent> ent, ref ToolRefineDoAfterEvent args)
    {
        if (args.Used is { } used)
            ent.Comp.ActiveTools.Remove(used);
        ...
        var logButchering = TryComp<ButcherableComponent>(ent, out var butcherable) &&
        ...
        if (logButchering)
        {
            _adminLogger.Add(LogType.Gib,
                ...
                $"has butchered {ToPrettyString(ent):target} " +
    ...
    {
        component.ActiveTools.RemoveWhere(uid => TerminatingOrDeleted(uid));

        if (!component.ActiveTools.Add(used))
            return false;
        ...
        var result = _toolSystem.UseTool(
            ...
            duplicateCondition: DuplicateConditions.SameTool |
                                ...
                                DuplicateConditions.SameEvent
        );

        if (id == null)
            component.ActiveTools.Remove(used);
...
{
    /// <summary>
    /// Whether this attempt is the completion-time revalidation.
    /// </summary>
