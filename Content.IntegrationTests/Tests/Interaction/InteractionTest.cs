...
";
    ...
    public override async Task DoSetup()
    {
        ...
        NetEntity? oldNet = default;
        await Client.WaitPost(() => CEntMan.TryGetNetEntity(cPlayerMan.LocalEntity, out oldNet));
        ...
        await Server.WaitPost(() =>
        {
            ...
            if (SEntMan.TryGetEntity(oldNet, out var oldUid))
                old = oldUid;
        ...
        if (old is { } oldUid)
            await Pair.DeleteEntityTreeLeafFirst(oldUid);
    ...
    {
        // TestPair.Cleanup disconnects clients before deleting TestMap, avoiding a replicated
        // parent/child deletion batch while the client still has the hierarchy.
