...
public sealed class StorageTest : GameTest
{
    ...
    public async Task TestSufficientSpaceForFill()
    {
        ...
        // RMC14 storage grids expand automatically, and some storage ignores the size of whitelisted contents.
        ...
        var entityWhitelistSys = entMan.System<EntityWhitelistSystem>();
        var itemSys = entMan.System<SharedItemSystem>();
        ...
        await Assert.MultipleAsync(async () =>
        {
            foreach (var (proto, fill) in pair.GetPrototypesWithComponent<StorageFillComponent>())
            {
                ...
                IgnoreContentsSizeComponent? ignoreContentsSize = null;
                ...
                await server.WaitAssertion(() =>
                {
                    ...
                    proto.TryComp(out ignoreContentsSize, compFact);
                ...
                // RMC14: the grid is automatically expanded to fit its configured fill.
